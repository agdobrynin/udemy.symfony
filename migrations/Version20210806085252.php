 ...
 */
final class Version20210806085252 extends AbstractMigration
{
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IDX_9474526CF675F31B ON comment (author_id)');
        $this->addSql('CREATE INDEX IDX_9474526C4B89032C ON comment (post_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, login, password, email, roles, name, password_changed_time_stamp FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, login VARCHAR(20) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, roles CLOB NOT NULL --(DC2Type:simple_array)
        , name VARCHAR(255) NOT NULL, password_changed_time_stamp INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, login, password, email, roles, name, password_changed_time_stamp) SELECT id, login, password, email, roles, name, password_changed_time_stamp FROM __temp__user');
